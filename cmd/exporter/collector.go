...
var (
	...
	handleTrafficLabels = []string{"tc_handle", "tc_handle_name"}
...
type tcCpumapCollector struct {
	...
	handleTrafficLocalRxBytes   *prometheus.Desc
	handleTrafficLocalRxPackets *prometheus.Desc
	handleTrafficLocalTxBytes   *prometheus.Desc
	handleTrafficLocalTxPackets *prometheus.Desc
...
func newTcCpumapCollector(logger *slog.Logger, tcHandleNames tc.TcHandleNames) *tcCpumapCollector {
	return &tcCpumapCollector{
		...
		handleTrafficLocalRxBytes:   handleTrafficLocalRxBytes,
		handleTrafficLocalRxPackets: handleTrafficLocalRxPackets,
		handleTrafficLocalTxBytes:   handleTrafficLocalTxBytes,
		handleTrafficLocalTxPackets: handleTrafficLocalTxPackets,
...
func (collector *tcCpumapCollector) Describe(ch chan<- *prometheus.Desc) {
	...
	ch <- collector.handleTrafficLocalRxBytes
	ch <- collector.handleTrafficLocalRxPackets
	ch <- collector.handleTrafficLocalTxBytes
	ch <- collector.handleTrafficLocalTxPackets
	...
	for _, bpfMap := range bpfMaps {
		...
		if bpfMap == "map_traffic_local" {
			for handle, aggData := range handleAggData {
				...
				ch <- prometheus.MustNewConstMetric(
					collector.handleTrafficLocalRxBytes,
					prometheus.CounterValue,
					float64(aggData.RxBytes),
					tcHandleString,
					tcHandleName,
				...
				ch <- prometheus.MustNewConstMetric(
					collector.handleTrafficLocalRxPackets,
					prometheus.CounterValue,
					float64(aggData.RxPackets),
					tcHandleString,
					tcHandleName,
				...
				ch <- prometheus.MustNewConstMetric(
					collector.handleTrafficLocalTxBytes,
					prometheus.CounterValue,
					float64(aggData.TxBytes),
					tcHandleString,
					tcHandleName,
				...
				ch <- prometheus.MustNewConstMetric(
					collector.handleTrafficLocalTxPackets,
					prometheus.CounterValue,
					float64(aggData.TxPackets),
					tcHandleString,
					tcHandleName,
