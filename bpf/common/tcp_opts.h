...
static __always_inline int parse_tcp_ts(struct tcphdr *tcph, void *data_end,
  ...
  if ((void *)(tcph + 1) > data_end || len <= sizeof(struct tcphdr))
    return -1;
#pragma unroll // temporary solution until we can identify why the non-unrolled
               // loop gets stuck in an infinite loop
  for (i = 0; i < MAX_TCP_OPTIONS; i++) {
    ...
    if (opt == 8 && opt_size == 10) {
      if ((void *)(pos + 10) > opt_end || (void *)(pos + 10) > data_end)
        return -1;
